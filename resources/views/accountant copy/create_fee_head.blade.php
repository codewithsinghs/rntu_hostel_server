...
<div class="container mt-5">
    <h2 class="mb-4">Create Fee Head</h2>
        ...
        $('#feeHeadForm').on('submit', function(event) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        showAlert("success", response.message || "Fee Head created successfully!");
                        window.location.href = "{{ route('accountant.fee_heads') }}"; // Redirect to fee heads list
                    } else {
                        showAlert("danger", response.message || "Failed to create Fee Head.");
                    }
                },
                error: function(xhr) {
                    if (xhr.status === 422) { // Validation error
                        ...
                        showAlert("danger", "An error occurred while creating the fee head.");
                    }
                }
            });
        });
