        ...
        <div class="common-content">
            <div class="common-overview">
                <div class="top-breadcrumbs d-flex justify-content-between align-items-center">
                    <div class="breadcrumbs p-0"><a class="p-0">Department List</a></div>
        ...
        <script>
            ...
            const RecordsModal = {
                ...
                openEdit(id) {
                    RecordsForm.reset();
                    this.setMode('edit');
                    const url = "{{ route('departments.show', ':id') }}".replace(':id', id);
                    $.ajax({
                        url,
                        headers: {
                        ...
                        success: res => {
                            ...
                            $('#name').val(d.name);
                            $('#code').val(d.code ?? '');
                            FacultySelect.load(d.faculty_id);
                            $('#status').val(d.status);
                            $('#recordsModal').modal('show');
                        },
