...
class LeaveTableHandler {
    ...
    setupEventListeners() {
        ...
        this.tableBody?.addEventListener('click', (e) => {
            ...
            if (target.classList.contains('view-receipt-btn')) {
                ...
                document.dispatchEvent(new CustomEvent('viewReceipt', { detail: leaveId }));
            }
            
            if (target.classList.contains('edit-leave-btn')) {
                const leaveId = target.dataset.id;
                document.dispatchEvent(new CustomEvent('editLeave', { detail: leaveId }));
            }
        });
    }
    ...
    getStatusBadge(status) {
        ...
        const statusInfo = statusMap[status.toLowerCase()] || { class: 'bg-info', label: status };
