...
@section('content')
    ...
    <script>
        ...
        //     // Set current date as applied date
        //     const now = new Date();
...
@push('scripts')
            ...
            init() {
                ...
                recordsTable = $('#recordsTable').DataTable({
                    ...
                    //                 `
                        ...
                        //         </button>
                        //               <button class="btn btn-sm btn-outline-success gatepass-btn" data-id="${data.id}">
    ...
    const RecordsModal = {
        ...
        openView(id) {
            ...
            $.ajax({
                ...
                success: res => {
                    ...
                    $('#recordForm input, #recordForm select, #recordForm textarea').prop('disabled',
                        true);
        ...
        delete(id) {
            Swal.fire({
                ...
                const url = "{{ route('resident.leaves.destroy', ':id') }}".replace(':id', id);
        ...
        cancelLeave(id) {
            ...
            Swal.fire({
                ...
                const url = "{{ route('resident.leaves.cancel', ':id') }}".replace(':id', id);
                ...
                $.ajax({
                    ...
                    success: res => {
                        Swal.fire({
                            icon: 'success',
                            title: 'Cancelled!',
                            text: res.message ?? 'Leave request has been cancelled.',
                            timer: 3000,
    ...
    const RecordsForm = {
        ...
        init() {
            ...
            $('#recordsForm').on('change', '[name="start_date"], [name="end_date"]', this.calculateDuration.bind(
                this));
        ...
        checkAttachmentRequirement() {
            ...
            const mandatoryCases = {
                ...
                reason: ['doctor_appointment', 'hospital_visit', 'accident', 'document_work', 'govt_office'],
            ...
            if (isRequired) {
                ...
                $attachmentInput.closest('.mb-4').find('small')
                    .text('Upload supporting documents like medical certificate, invitation, etc. (Max: 5MB)');
            }
        },
        ...
        updateReasonOptions() {
            ...
            if (type) {
                ...
                reasons.forEach(reason => {
                    $reasonSelect.append(`<option value="${reason.value}">${reason.label}</option>`);
                });
                ...
                $reasonSelect.prop('disabled', true).empty().append('<option value="">Select Type First</option>');
        ...
        calculateDuration() {
            ...
            if (end < start) {
                $durationField.html('<span class="text-danger">End date cannot be before start date!</span>');
        ...
        validate() {
            ...
            if (type === 'emergency') {
                if (!emergencyContact || emergencyContact.trim() === '') {
                    this.error('emergency_contact', 'Emergency contact is required for emergency leaves');
                    ok = false;
        ...
        reset() {
            ...
            $('[name="reason"]').empty().append('<option value="">Select Type First</option>').prop('disabled',
                true);
            ...
            // Uncheck the declaration checkbox
    ...
    const LeaveActions = {
        ...
        document.getElementById('leaveActionForm').addEventListener('submit', function(e) {
            ...
            const url = "{{ route('resident.leaves.update', ':id') }}".replace(':id', id);
