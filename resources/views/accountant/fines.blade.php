            ...
            <div class="modal-content">
                <form id="fineForm">
                    ...
                    <div class="modal-body">
                        ...
                        <p><strong>Proposed Amount:</strong> <span id="modalPropAmount"></span></p>
                        <p><strong>Fine Date:</strong> <span id="modalFineDate"></span></p>
                        <p><strong>Admin Remarks:</strong> <span id="modalAdminRemarks"></span></p>


                        <div class="mb-3">
                            <label class="form-label">Final Payable Amount</label>
...
<script>
    document.addEventListener("DOMContentLoaded", async () => {
        ...
        async function loadFineDetails() {
            try {
                ...
                if (result.success && Array.isArray(result.data) && result.data.length > 0) {
                    ...
                    `;
                        ...
                        li.querySelector('.view-btn').addEventListener("click", () => {
                            ...
                            document.getElementById("modalResidentName").textContent =
                                fine.resident_name;
                            document.getElementById("modalScholarNo").textContent = fine
                                .scholar_no;
                            document.getElementById("modalAdminRemarks").textContent =
                                fine.admin_remarks || "N/A";
                            document.getElementById("modalPropAmount").textContent =
                                fine.proposed_amount || "N/A";
                            document.getElementById("modalFineDate").textContent = fine
                                .fine_date || "N/A";

                            document.querySelector("#fineForm [name='resident_id']")
                                .value = fine.resident_id;
                            document.querySelector("#fineForm [name='fine_id']").value =
                                fine.fine_id;
                    ...
                    residentList.innerHTML =
                        `<li class="list-group-item text-danger">No fines found.</li>`;
                }
            } catch (err) {
                console.error(err);
                residentList.innerHTML =
        ...
        document.getElementById("fineForm").addEventListener("submit", async function(e) {
            ...
            const form = e.target;
            const data = {
                fine_id: parseInt(form.fine_id.value),
                payable_amount: parseFloat(form.payable_amount.value),
                payment_method: "Null",
                created_by: parseInt(form.created_by.value) || localStorage.getItem(
                    'auth-id'),
            ...
            try {
                ...
                const result = await res.json();
                console.log('fine update'.result);
                if (result.success) {
                    alert("Fine submitted successfully.");
                    form.reset();
                    fineModal.hide();
                    await loadFineDetails(); // Refresh data
                } else {
