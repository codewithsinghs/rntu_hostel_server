...
@push('scripts')
    <script>
        $(document).ready(function() {
            ...
            fillFormWithRoomData(id);

        });

        function fillFormWithRoomData(id) {
            $.ajax({
                url: `/api/admin/rooms/${id}`, // Adjust the URL as per your API endpoint
                type: "GET",
                headers: {
                ...
                success: function(response) {
                    if (response.success && response.data) {
                        ...
                        updateFloorDropdown(getBuildingFloorsById(room.building_id));
                        $("#floorSelect").val(room.floor_no);
                        $("#status").val(room.status);
                    } else {
                        showMessage("danger", "Room not found or invalid data.");
                    }
                },
                error: function(xhr) {
                    ...
                    console.error("Error fetching room data:", xhr);
                }
            });
        }
                    ...
                    if (response.success && Array.isArray(response.data)) {
                        response.data.forEach(function(building) {
                            const option = $("<option></option>")
                                .val(building.id)
                                .text(building.name)
                                .data("floors", building
                                .floors); // Store the number of floors in the option
                            buildingSelect.append(option);
                        });
                    } else {
                        showMessage("danger",
                            "Invalid response from server for buildings. Expected an array in 'data' field."
                            );
        ...
        function updateRoom(id) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        showMessage("success", response.message || "Room Updated successfully!");
                        $("#createRoomForm")[0].reset(); // Reset the form
                        $("#floorSelect").empty().append(
                        '<option value="">-- Select Floor --</option>'); // Reset floor dropdown
