...
@section('content')
    ...
    <div class="container mt-4">
        ...
        <p id="error-message" class="text-danger text-center mt-3" style="display: none;">Error loading data, please try
            again.</p>
    </div>
            ...
            fetch("/api/resident/pending/appliedFines", {
                ...
                .then(data => {
                    ...
                    const rows = data.invoices.map(invoice => {
                        ...
                        invoiceItemMap[invoice.id] = items;

                        const total = items.reduce((sum, item) => sum + parseFloat(item.total_amount),
                            0);
                        const paid = items.reduce((sum, item) => sum + parseFloat(item.amount_paid ||
                            0), 0);
                        const remaining = total - paid;

                        const payButton = remaining > 0 ?
                            `<button class="btn btn-danger btn-sm" onclick="initiateFinePayment(${invoice.id}, ${remaining})">Pay Now</button>` :
