...
@push('scripts')
    ...
    <script type="text/javascript">
$(document).ready(function () {
    displayError('scholar_no', 'Scholar number is required.');
    ...
    // Convert snake_case field name → camelCase ID
    let camelCase = field.replace(/_([a-z])/g, function (g) {
...
function displayError(field, message) {
    ...
    let errorEl = inputEl.closest('.mb-3').find('.invalid-feedback');

    inputEl.addClass('is-invalid');
    if (errorEl.length) {
        errorEl.text(message).show(); // ensure text is visible
    }
}
...
function clearError(field) {
    ...
    $('#registrationForm').submit(function (e) {
        ...
        if (!$('input[name="room_preference"]:checked').length) {
            displayError('room_preference', 'Please select your bed preference.');
            valid = false;
        }
        ...
        if (!valid) {
            showAlert("error", "Form Validation Failed", "Please correct highlighted errors.");
            return;
        }
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.success) {
                    ...
                    showAlert("warning", "Registration Failed", response.message || "Something went wrong.");
                }
            },
            error: function (xhr) {
                $('#loading').addClass('hidden');
                ...
                if (response && response.errors) {
                    ...
                    showAlert("error", "Validation Errors", "Please check highlighted fields.");
                } else {
                    showAlert("error", "Server Error", "An unexpected error occurred. Please try again.");
                }
            }
        });
    });
    ...
    $('#faculty_id').change(function () {
        ...
        $.getJSON('/api/accessories/active/' + facultyId, function (data) {
            ...
            showAlert("error", "Accessories Error", "Unable to load accessories.");
        });
    });
