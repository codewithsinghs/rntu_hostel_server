...
<div class="container mt-5">
    ...
    <div class="d-flex justify-content-between mt-5 mb-3">
        <h2>Assign Accessory to Resident</h2>
    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', async function() {
        ...
        try {
            // Fetch residents
            const residentResponse = await fetch("{{ url('/api/admin/residentswarden') }}", {
                method: 'GET',
                headers: {
            ...
            accessories.forEach(accessory => {
                ...
                option.setAttribute('data-accessory-head-id', accessory
                    .accessory_head_id); // Store accessory_head_id
        ...
        document.getElementById('sendAccessoryForm').addEventListener('submit', async function(e) {
            ...
            const selectedOption = document.querySelector(
                `#accessory_id option[value="${accessoryId}"]`);
            const accessoryHeadId = selectedOption.getAttribute(
                'data-accessory-head-id'); // Get accessory_head_id
