...
class CheckoutController extends Controller
{
    ...
    //         $active = Checkout::where('resident_id', $resident->id)
    //             ->whereIn('status', ['pending', 'approved'])
    //             ->exists();

    //         if ($active) {
    //             return response()->json([
    //                 'message' => 'You already have an active checkout request.'
    //             ], 422);
    ...
    public function requestCheckout(Request $request)
    {
        ...
        try {
            ...
            // 2️⃣ FETCH RESIDENT
            // ---------------------------------------
            $resident = Helper::get_resident_details($userId);
            ...
            // ---------------------------------------
            // 4️⃣ PREVENT DUPLICATE CHECKOUT
            // ---------------------------------------

            $hasActiveCheckout = Checkout::where('resident_id', $resident->id)
            // ->whereIn('status', ['pending', 'approved'])
                ->where(function ($q) {
                    $q->whereIn('status', ['pending', 'approved'])
                        ->orWhere('action', ['pending', 'approved']);
                })
                ->exists();
            ...
            // ---------------------------------------
            // 5️⃣ CALCULATE CAUTION MONEY
            // ---------------------------------------
            $cautionAmount = Payment::where('resident_id', $resident->id)
                ->where('is_caution_money', true)
                ->sum('amount');

            // ---------------------------------------
            // 6️⃣ CREATE CHECKOUT REQUEST
            // ---------------------------------------
            $checkout = Checkout::create([
            ...
            return response()->json([
                ...
                'data'    => [
                    ...
                    'deposited_amount' => $checkout->deposited_amount,
                ],
                'errors'  => null
            ], 201);
            ...
            return response()->json([
                'success' => false,
                'message' => 'Something went wrong while submitting the checkout request.',
