...
class GuestController extends Controller
{
    ...
    // {
    //     try {
    //         $guest = Guest::with('accessories')->findOrFail($guest_id);

    //         // Get latest invoice with all items (fee + accessory)
    //         $invoice = $guest->invoices()
    //             ->latest()
    ...
    public function guestDetails($guest_id)
    {
        try {
            ...
            // Load guest + accessories
            $guest = Guest::with('accessories')->findOrFail($guest_id);

            // Load latest invoice with items
            $invoice = $guest->invoices()
                ->latest()
                ->with(['items' => function ($q) {
                    $q->select(
            ...
            // Log::info('items'. json_encode($invoice));

            // Attach invoice data to the guest object
            $guest->invoice = $invoice;

            return response()->json([
                'success' => true,
                'message' => 'Guest details fetched successfully',
                'data' => $guest,   // 👈 frontend remains unchanged
                'errors' => null
            ]);
        } catch (ModelNotFoundException $e) {
            return response()->json([
